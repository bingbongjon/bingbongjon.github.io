...
function Intro({ id, sectionRef, disciplines, scrollIndicatorHidden, ...rest }) {
    ...
    return ( <
        ...
        tabIndex = {-1 } {...rest } >
        <
        Transition key = { theme.themeId }
        appear = {!prerender } in = {!prerender }
        timeout = { 3000 }
        onEnter = { reflow } >
        {
            status => ( <
                ...
                <
                h1 className = { classNames('intro__name', `intro__name--${status}`) }
                id = { titleId } >
                <
                DecoderText text = "Cody Bennett"
                start = {!prerender }
                delay = { 300 }
                /> <
                /h1> <
                ...
                component = "span" >
                {
                    currentDisciplines.map(item => ( <
                        Transition appear timeout = {
                            { enter: 3000, exit: 2000 } }
                        key = { item }
                        onEnter = { reflow } >
                        {
                            wordStatus => ( <
                    ...
                    windowSize.width > media.tablet && ( <
                        div className = {
                            classNames(
                                ...
                                `intro__scroll-indicator--${status}`, { 'intro__scroll-indicator--hidden': scrollIndicatorHidden }
                            )
                        }
                        status = { status }
                        />
                    ...
                    windowSize.width <= media.tablet && ( <
                        div className = {
                            classNames(
                                ...
                                `intro__mobile-scroll-indicator--${status}`, { 'intro__mobile-scroll-indicator--hidden': scrollIndicatorHidden }
                            )
                        } >
                        <
